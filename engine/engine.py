...
class Engine:
    ...
    def draw_frame(self) -> None:
        ...
        for obj in self.eng_objects:
            obj.draw(self.ax, self.frame_clock)
    ...
    def run(self, fps_target: int = 24) -> None:
        """
        Re-draws the same frame 'frames' times.
        Useful if you want a persistent window without updates.
        """
        target_frame_time: float = 1.0 / fps_target   # seconds per frame
        plt.ion()
        ...
        while True:
            ...
            actual_fps = 1.0 / actual_frame_time if actual_frame_time > 0 else float('inf')
